---
{"dg-publish":true,"permalink":"/aibn-porosity-measurement-and-adsorption-isotherms/","dg-note-properties":{}}
---
...
Type IV isotherms are related but tend to level out at higher pressures, with an additional hysteresis loop, of which the lower branch corresponds to adsorption, the upper to desorption. This is due to capillary condensation in the mesopores, a process where vapour condenses into liquid below the saturation vapour pressure thanks to increased number of van der Waals interactions between vapour phase molecules inside the confined space. This is an issue notably in atomic force microscopy (AFM). 

### Freundlich isotherm : a model of type II
...
- A ${\displaystyle \Delta _{\mathrm {r} }H}$ that varies with site occupancy indicates an inhomogeneous surface and therefore the use of the Freundlich isotherm

A real type II reaches a saturation point for the first monolayer, and then proceeds to form additional adsorbate layers.

### Type III isotherm

Type III isotherms usually occur in systems with weak adsorbent-adsorbate interactions and strong adsorbate-adsorbate interactions, often accompanied with a heterogeneous deposition morphology.

Type V isotherms resemble type III but with an additional hysteresis loop associated with a pore filling mechanism.

### Type VI isotherm

Type VI isotherms are rare, and usually represent successive multi-layer adsorption on uniform non-porous surfaces, where step height is proportional to capacity for each layer.

---
### Gurvich rule

- Hypothesis: horizontal plateau of an isotherm represents the region where pore filling is complete and that fluid adsorbed in the pores has the same density as the bulk liquid at the same temperature and pressure.

The following is therefore true: $$V_{tot} = \dfrac{m^{sat}_{ads}}{\rho_{liq}}= \dfrac{V_{STP}M p}{\rho R T} = V_{STP} × 1.547 × 10^{−3}$$When using nitrogen, $V_{tot}$ is total pore volume, M molar mass of $N_2$, p is pressure at STP
